...
// const orderLive = inventors.sort(function (a, b) {
//   const aAge = a.passed - a.year;
//   const bAge = (b.passed = b.year);
//   return aAge > bAge ? -1 : 1;
// });

const orderLive = inventors.sort((a, b) => {
  const aAge = a.passed - a.year;
  const bAge = (b.passed = b.year);
  return aAge > bAge ? -1 : 1;
});

console.table(orderLive);
...
const sumNum = data.reduce((acc, item) => {
  if (!acc[item]) {
    acc[item] = 0;
  }
  acc[item]++;
